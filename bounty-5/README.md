...
2. **Configure** the `Config` node: set `githubRepo` (`owner/repo`), `slackWebhookUrl`, and `language` (`EN` or `FR`). The workflow uses `claude-sonnet-5`.
...
- Calls Anthropic's Messages API with `claude-sonnet-5`.
...
| `claudeModel` | `claude-sonnet-5` | Claude model used for the summary |

## Model-availability note

Issue #5 names `claude-sonnet-4-20250514`, which Anthropic retired after June 15, 2026. This workflow uses its current replacement, `claude-sonnet-5`. To use a different model, change `Config.claudeModel`.
